...
``||Advanced:ADVANCED||`` ドロワーを画面左から開き、次に ``||Builder:BUILDER||`` ドロワーを画面左から開きます。``||Builder:BUILDER||`` ドロワーを使うと、Minecraftのポジション機能にアクセスして操作できます。``||Builder:BUILDER||`` を、目に見えない ``||Agent:Agent||``. 
