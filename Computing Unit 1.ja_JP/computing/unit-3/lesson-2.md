...
## ステップ5
``||Agent:agent turn [left]||`` コードブロックを、1つ目(外側)の **outer** ``||Loops:repeat [4] times||`` ループの内側、かつ2つ目(内側)の ``||Loops:repeat [4] times||``.
...
## ステップ8
手順を繰り返しますが、新しいセットは ``||Agent:agent set active slot||`` の後ろに配置し、``||Agent:agent destroy [down]||`` だけを ``||Agent:agent place [down]||``.  
